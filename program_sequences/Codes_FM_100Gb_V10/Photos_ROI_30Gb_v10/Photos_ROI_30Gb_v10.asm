// ; EEPROM Address - 
// ;Photos ROI images in three filter combinations 
// ;for 6 hours duration, flare routines as usual (default)
// ;will produce about 30 Gb data (before compression)
// ;This program uses four observation IDs [7D,7E,7F and F7]
...
// ;v9  MOVE_FW2	00 added on line 64
//
		CLEAR_FLAG						// Clear all Flare Flags 	
//
		SET_CYCLE	02D0				// ;Repeat 720 times, each loop ~ 30 sec.
//
...
//
		...
		CCD_START						// 01 min dark, 2x2 binned image
		CNC
BEGIN:	SET_OBSERV_ID	7D				// Program Observation ID for NORMAL Mode 
		SET_FRAME_SIZE	00				// Full-Frame
...
//
NORMAL_MODE:	SET_FRAME_SIZE	01			// ROI
		SET_BIN	00						// No Binning
		...
		SET_EXP_TIME	51C8			// LUT address, 1400 ms exposure
		CCD_START						// start CCD sequencer
//1		
		CNC								// Check and Execute TC
...
//
		...
		SET_LED	2A43	022A	4427		// Update Shutter Offset value
		HOME_SHU						// Shutter homing
		SET_EXP_TIME	51D8			// LUT address, 1400 ms exposure
		CCD_START
//2		
		CNC								// Check and Execute TC
		JIF	JUMP_EXT					// check for EXT FLARE Trigger
...
//
NORM_FLARE:	SET_OBSERV_ID	7E			// ;Observation ID for Normal Flare
//
...
//							
PROM_FLARE:	SET_OBSERV_ID	7F			// ;Observation ID for Prom. Flare
		SET_FRAME_SIZE	01				// ROI
		ROI_UpdateEn
//
		SET_BIN	00						// No binning
