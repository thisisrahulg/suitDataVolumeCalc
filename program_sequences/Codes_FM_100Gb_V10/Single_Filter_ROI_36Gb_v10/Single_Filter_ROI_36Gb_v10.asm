// ; EEPROM Address - 
// ; Single Filter (NB8 ?) ROI images for 6 hours duration
// ; Binned image taken after every ROI image for flare detection
// ; Flare Routines as Usual
// ; will produce about 36 Gb data (before compression)
// ; flare routine time corrected by setting seq = 120 (0x78)
// ; Program uses four observation IDs [80,81,82 and F7]
// ; One (ROI + BIN) loop of ~ 22 sec. repeated 981 times
...
// ; v9 - MOVE_FW2	00 added on line 56
//
		CLEAR_FLAG						// Clear all Flare Flags 	
//
		SET_CYCLE	03D5				// ;Repeat 981 times, each loop ~ 22 sec.
//
...
//
		...
		CCD_START						// 01 min dark, 2x2 binned image
		CNC
BEGIN:	SET_OBSERV_ID	80				// Program Sequence ID for NORMAL Mode observations
		SET_FRAME_SIZE	00
...
//
NORMAL_MODE:	SET_FRAME_SIZE	01			// ROI FRAME
		SET_BIN	00						// No Binning
		MOVE_FW1	05					// say, NB8
		MOVE_FW2	03					// NB8
		SET_EXP_TIME	51D0			
...
//
NORM_FLARE:	SET_OBSERV_ID	81			// ;Observation ID for Normal Flare
//
...
//							
PROM_FLARE:	SET_OBSERV_ID	82			// ;Observation ID for Prom. Flare
		SET_FRAME_SIZE	01				// ROI
		ROI_UpdateEn
//
		SET_BIN	00						// No binning
